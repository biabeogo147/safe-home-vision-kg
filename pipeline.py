...
from vision.detector import Detection, YoloV26Detector
...
def build_detector(config: dict[str, Any] | None = None) -> YoloV26Detector:
    config = config or {}
    label_mapper = config.get("label_mapper") or _load_label_mapper_from_env()

    detector_name = str(config.get("detector") or os.getenv("DETECTOR", "yolov26")).lower()
    if detector_name != "yolov26":
        raise ValueError(f"Unsupported detector: {detector_name}. Only 'yolov26' is available.")
